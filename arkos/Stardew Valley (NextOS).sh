#!/usr/bin/env bash
# Universal PortMaster/NextOS entry point for Stardew Valley Android 1.6.15.3.
...
GAMEDIR=
if [ -n "${directory:-}" ] &&
   [ -d "/${directory#/}/ports/sdvnextos" ]; then
  GAMEDIR="/${directory#/}/ports/sdvnextos"
elif [ -d "$SCRIPT_DIR/sdvnextos" ]; then
  GAMEDIR=$SCRIPT_DIR/sdvnextos
elif [ -d /roms/ports/sdvnextos ]; then
  GAMEDIR=/roms/ports/sdvnextos
elif [ -d /roms2/ports/sdvnextos ]; then
  GAMEDIR=/roms2/ports/sdvnextos
elif [ -d /storage/roms/ports/sdvnextos ]; then
  GAMEDIR=/storage/roms/ports/sdvnextos
elif [ -n "${directory:-}" ] &&
     [ -d "/${directory#/}/ports/stardewvalley" ]; then
  GAMEDIR="/${directory#/}/ports/stardewvalley"
elif [ -d "$SCRIPT_DIR/stardewvalley" ]; then
  GAMEDIR=$SCRIPT_DIR/stardewvalley
elif [ -d /roms/ports/stardewvalley ]; then
  GAMEDIR=/roms/ports/stardewvalley
elif [ -d /storage/roms/ports/stardewvalley ]; then
  GAMEDIR=/storage/roms/ports/stardewvalley
else
  launcher_error "game directory not found"
fi
GAMEDIR=$(CDPATH= cd -- "$GAMEDIR" 2>/dev/null && pwd -P) ||
  launcher_error "game directory is inaccessible"
