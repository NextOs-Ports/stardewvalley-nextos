...
int so_load(const char *filename, void *base, size_t max_size) {
  ...
  /* Todos os callers deste port fornecem um mmap MAP_ANONYMOUS novo, portanto
   * a regiao (inclusive BSS) ja vem zerada pelo kernel. Tocar o span inteiro
   * aqui materializava inutilmente ~17 MiB do enorme BSS de libxamarin-app no
   * RSS do device de 832 MiB. Os PT_LOAD abaixo continuam copiando todos os
   * bytes reais do arquivo. */
...
err_free_load:
  /* load_base pertence ao caller (neste port, sempre um mmap). Liberar aqui
   * com free() mistura allocators e pode abortar justamente no caminho de
   * erro. O caller desfaz o mmap depois que so_load() retorna falha. */
err_free_so:
  free(so_base);
  return res;
}
